---
title: "[Next 13] 이미지 최적화"
description: 내장 `next/image` 컴포넌트를 사용하여 이미지를 최적화합니다.
slug: next-13-optimizing-images
date: Oct 19, 2023
---
...
Next.js는 빌드 과정 중 원격 파일에 대해 접근하지 않기 때문에, [`width`](https://nextjs.org/docs/app/api-reference/components/image#width), [`height`](https://nextjs.org/docs/app/api-reference/components/image#height) 및 선택적인 [`blurDataURL`](https://nextjs.org/docs/app/api-reference/components/image#blurdataurl) 속성을 수동으로 제공해야 합니다.
...
[`remotePatterns`](https://nextjs.org/docs/app/api-reference/components/image#remotepatterns) 구성에 대해 배워보세요. 만약 이미지 `src`에 상대 URL을 사용하려면, [`loader`](https://nextjs.org/docs/app/api-reference/components/image#loader)를 사용하세요.
...
> [`remotePatterns`](https://nextjs.org/docs/app/api-reference/components/image#remotepatterns) 구성에 대해 배워보세요.
...
[`loader` 속성](https://nextjs.org/docs/app/api-reference/components/image#loader)로 이미지별 로더를 정의할 수 있습니다. 또는 [`loaderFile` 구성](https://nextjs.org/docs/app/api-reference/components/image#loaderfile)으로 애플리케이션 수준으로 정의가 가능합니다.
...
priority에 대해 더 알아보려면 [`next/image` 컴포넌트 문서](https://nextjs.org/docs/app/api-reference/components/image#priority)를 참조하세요.
...
2. [`width`](https://nextjs.org/docs/app/api-reference/components/image#width)와 [`height`](https://nextjs.org/docs/app/api-reference/components/image#height) 속성을 포함하여, 명시적으로 설정하는 방법
3. [fill](https://nextjs.org/docs/app/api-reference/components/image#fill) 속성을 사용하여 암시적으로 상위 요소의 크기로 설정하는, 암시적인 방법
...
- `styled-jsx`가 아닌 `className` 또는 `style`를 사용하세요.
  - 대부분의 경우에, `className` 속성을 사용하는 것을 권장합니다. 이는 가져온 [CSS Module](https://nextjs.org/docs/app/building-your-application/styling/css-modules), 또는 [global stylesheet](https://nextjs.org/docs/app/building-your-application/styling/css-modules#global-styles) 등이 될 수 있습니다.
  - 또한 인라인 스타일을 적용하기 위해 `style` 속성을 사용할 수 있습니다.
  - [styled-jsx](https://nextjs.org/docs/app/building-your-application/styling/css-in-js)를 사용할 수 없습니다. 스타일을 `global`로 표시하지 않는 한 현재 컴포넌트에 대해 스코프가 지정되기 때문입니다.
...
<img
  ...
  src="https://nextjs.org/docs/light/responsive-image.png"
  width="1600"
  height="629"
/>
...
<img
  ...
  src="https://nextjs.org/docs/light/fill-container.png"
  width="1600"
  height="529"
/>
...
<img
  ...
  src="https://nextjs.org/docs/light/background-image.png"
  width="1600"
  height="427"
/>
...
[**`next/image` 컴포넌트에서 사용할 수 있는 모든 속성 보기.**](https://nextjs.org/docs/app/api-reference/components/image)

## 구성

`next/image` 컴포넌트와 Next.js Image Optimization API는 [`next.config.js` file](https://nextjs.org/docs/app/api-reference/next-config-js)에서 구성할 수 있습니다. 이 구성을 통해 [원격 이미지 사용](https://nextjs.org/docs/app/api-reference/components/image#remotepatterns), [커스텀 이미지 중단점](https://nextjs.org/docs/app/api-reference/components/image#devicesizes), [캐싱 동작 변경](https://nextjs.org/docs/app/api-reference/components/image#caching-behavior) 등의 작업을 수행할 수 있습니다.

[**자세한 내용은 전체 이미지 구성 문서를 참조하세요.**](https://nextjs.org/docs/app/api-reference/components/image#configuration-options)
